...
function response()
{
	...
	if($req)
	{
		...
		$res["text"] = 'Last price : ' . sprintf("%.8f", $characters->result[0]->Last) . ' <br /> Hight price: ' . sprintf("%.8f", $characters->result[0]->High) . ' <br /> Low price: ' . sprintf("%.8f", $characters->result[0]->Low) .' <br /> Volume ' . $characters->result[0]->Volume . ' <br /> BaseVolume ' . $characters->result[0]->BaseVolume;
		reply($req, $res);
	}
}
